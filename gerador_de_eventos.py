...
def cria_carro(linhas,idcarro,hora):
    ...
    carro['ts'] = time.time()
    carro['datahora'] = datetime.strftime(datetime.utcnow(),"%Y-%m-%dT%H:%M:%S")    
    ...
    carro['total_passageiros_meia'] = int(round(FATOR_FREQ_MAX[hora-1] * 0.3 * 50))
    carro['total_passageiros_isento'] = int(round(FATOR_FREQ_MAX[hora-1] * 0.1 * 50))
    carro['total_passageiros_integral'] = int(round(FATOR_FREQ_MAX[hora-1] * 0.6 * 50))
...
now = datetime.strftime(datetime.utcnow(),"%Y-%m-%dT%H:%M:%S")  
hora = int(datetime.strftime(datetime.utcnow(),"%H")  )
